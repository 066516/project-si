...
function ReglementClient({ setreglementClient }) {
  const fakeSuppliers = [
  ...
  const handlePaySalary = () => {
    console.log("Reglement Client created");
    // Add logic to create achat
  };
  const handleCancelpay = () => {
    console.log("Reglement Client canceled");
    setreglementClient(false);
  };

  return (
    <div className="relative bg-blue2/80 z-[100] w-screen h-screen flex justify-center items-start">
      <div className="bg-white relative top-3 p-5 rounded-xl">
        <h1 className="uppercase font-semibold">Reglement Client</h1>
        <ImCancelCircle
          onClick={() => setreglementClient(false)}
